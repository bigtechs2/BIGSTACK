...
function normalizeDuration(dur) {
    ...
    if (typeof dur === "number") {
        // If value > 10000, assume milliseconds
        if (dur > 10000) return Math.round(dur / 1000);
        return dur;
    }
...
async function download(url) {
    ...
    // ─── 2. Check cache ─────────────────────────────
    const cacheKey = `soundcloud:${trackId}`;
    const cached = await cache.get(cacheKey);
    if (cached) {
        logger.info(`[soundcloud] ✅ cache hit for ${trackId}`);
        return cached;
    }

    // ─── 3. Get providers ───────────────────────────
    const providers = config.getProviders("soundcloud");
    logger.info(`[soundcloud] processing ${trackId} across ${providers.length} providers`);
    ...
    for (const provider of providers) {
        try {
            logger.info(`[soundcloud] trying ${provider.name}...`);
            ...
            logger.info(`[soundcloud] ✅ ${provider.name} succeeded: "${result.title}"`);
            ...
            logger.warn(`[soundcloud] ❌ ${provider.name} failed: ${msg}`);
            lastError = err;
            continue;
        }
    }

    throw lastError || new Error("All soundcloud providers failed");
}

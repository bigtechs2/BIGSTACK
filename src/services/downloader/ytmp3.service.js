// ──────────────────────────────────────────────────
//  BIGSTACK — YTMP3 Service
//  Convert YouTube URL → MP3 audio
...
async function download(url) {
    ...
    if (cached) {
        logger.info(`[ytmp3] ✅ cache hit for ${videoId}`);
        return cached;
    }

    // ─── 3. Get providers ───────────────────────────
    const providers = config.getProviders("ytmp3");
    logger.info(`[ytmp3] processing ${videoId} across ${providers.length} providers`);
    ...
    for (const provider of providers) {
        try {
            logger.info(`[ytmp3] trying ${provider.name}...`);
            ...
            logger.info(`[ytmp3] ✅ ${provider.name} succeeded: "${result.title}"`);
            ...
            logger.warn(`[ytmp3] ❌ ${provider.name} failed: ${msg}`);
            lastError = err;
            continue;
        }
    }

    throw lastError || new Error("All ytmp3 providers failed");
}
